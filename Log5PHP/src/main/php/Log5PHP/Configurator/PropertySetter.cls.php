 ...
 * Usage:
 * <code>
 * $ps = new Log5PHP_Configurator_PropertySetter($anObject);
 * $ps->set("name", "Joe");
 * $ps->set("age", 32);
 * $ps->set("isMale", true);
 * </code>
 ...
 * @since 0.5
 */
class Log5PHP_Configurator_PropertySetter {
    ...
    /**
     * Create a new Log5PHP_Configurator_PropertySetter for the specified Object. 
    ...
    {
        $pSetter = new Log5PHP_Configurator_PropertySetter($obj);
    ...
    {
        Log5PHP_InternalLog::debug("Log5PHP_Utility_OptionConverter::setProperties():prefix=[{$prefix}]");
        ...
        while (list($key,) = each($properties)) {
            if (strpos($key, $prefix) === 0) {
                if (strpos($key, '.', ($len + 1)) > 0)
                    continue;
                $value = Log5PHP_Utility_OptionConverter::findAndSubst($key, $properties);
                $key = substr($key, $len);
    ...
    {
        Log5PHP_InternalLog::debug("Log5PHP_Utility_OptionConverter::setProperty():name=[{$name}]:value=[{$value}]");
    ...
    function activate()
    {
        Log5PHP_InternalLog::debug("Log5PHP_Utility_OptionConverter::activate()");
    
        if (method_exists($this->obj, 'activateoptions')) {
            return call_user_func(array($this->obj, 'activateoptions'));
        } else {
            Log5PHP_InternalLog::debug("Log5PHP_Utility_OptionConverter::activate() Nothing to activate.");
        }
    }
}
