 ...
 * @subpackage src_main_php_Log5PHP_Appender
 */
class Log5PHP_Appender_MailEvent extends Log5PHP_Appender_Base
{
    ...
    protected function append(Log5PHP_LogEvent $event)
    {
        ...
        Log5PHP_InternalLog :: debug("Log5PHP_Appender_MailEvent::append()" .
        ":from=[{$from}]:to=[{$to}]:smtpHost=[{$smtpHost}]:smtpPort=[{$smtpPort}]");

        if (!mail($to, $this->getSubject(), $this->layout->getHeader() . $this->layout->format($event) . $this->layout->getFooter($event), "From: {$from}\r\n"))
        {
            Log5PHP_InternalLog :: debug("Log5PHP_Appender_MailEvent::append() mail error");
        }
