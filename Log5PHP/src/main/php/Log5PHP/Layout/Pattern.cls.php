 ...
 */
define('LOG5PHP_LOGGER_PATTERN_LAYOUT_DEFAULT_CONVERSION_PATTERN', '%m%n');

define('LOG5PHP_LOGGER_PATTERN_LAYOUT_TTCC_CONVERSION_PATTERN', '%r [%t] %p %c %x - %m%n');

/**
 * A flexible layout configurable with pattern string.
 * 
 * <p>The goal of this class is to {@link format()} a {@link Log5PHP_LogEvent} and return the results as a string.
 ...
 * 
 * <p>Let the conversion pattern be "%-5p [%t]: %m%n" and assume that the log5php environment 
 * was set to use a Log5PHP_PatternLayout.</p> 
 * 
 * Then the statements
 * <code> 
 *  $root = Log5PHP_Manager::getRoot();
 ...
 *         It is recommended to use the predefined log5php date formatters. 
 ...
 *            See {@link Log5PHP_MDC} class for more details. 
 ...
 * @since 0.3 
 */
class Log5PHP_Layout_Pattern extends Log5PHP_Layout_Base
{
    ...
    /**
     * @var Log5PHP_PatternConverter head chain
     */
    private $head;
     ...
     * {@link LOG5PHP_LOGGER_PATTERN_LAYOUT_DEFAULT_LAYOUT_PATTERN}.
    ...
    function __construct($pattern = null)
    {
        if ($pattern === null)
        {
            $this->__construct(LOG5PHP_LOGGER_PATTERN_LAYOUT_DEFAULT_CONVERSION_PATTERN);
        }
        else
        {
    ...
    /**
     * Returns Log5PHP_PatternParser used to parse the conversion string. Subclasses
     * may override this to return a subclass of PatternParser which recognize
     * custom conversion characters.
     *
     * @param string $pattern
     * @return Log5PHP_PatternParser
     */
    private function createPatternParser($pattern)
    {
        return new Log5PHP_Utility_PatternParser($pattern);
    }
    ...
    {
        Log5PHP_InternalLog :: debug("Log5PHP_PatternLayout::format()");
        ...
        {
            $sbuf = $converter->format($sbuf, $event);
